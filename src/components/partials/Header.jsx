...
function Header({data}) {
  ...
  style={{
    background: `linear-gradient(rgba(0,0,0,.4), rgba(0,0,0,.7), rgba(0,0,0,.9)),
                url(https://image.tmdb.org/t/p/original/${data.poster_path || data.profile_path || data.backdrop_path})`,
  ...
  className="w-full h-[60vh] flex flex-col justify-end items-start p-[5%] ">
    ...
    <p className="w-[70%] text-white mt-3"> {data.overview.slice(0, 200)}...<Link className="text-blue-400">more</Link></p>
    ...
    <Link className=" bg-[#6556CD] p-3 rounded-lg mt-4 text-white font-semibold "> Watch Trailer
    </Link>
